...
"""Unit tests for LogAnalyzerService."""

from pathlib import Path

import pytest

from dv_agentic.tools.services import LogAnalyzerService


@pytest.fixture()
def agent() -> LogAnalyzerService:
    return LogAnalyzerService()


class TestErrorClassification:
    def test_uvm_fatal(self, agent: LogAnalyzerService) -> None:
    ...
    def test_xcelium_compile_error(self, agent: LogAnalyzerService) -> None:
    ...
    def test_ghdl_compile_error(self, agent: LogAnalyzerService) -> None:
        log = "ghdl: error: unexpected token ';'"
        s = agent.analyze(log)
        assert s.error_class == "compile_error"

    def test_uvm_error_single(self, agent: LogAnalyzerService) -> None:
        log = "Sim running...\nUVM_ERROR @ 50ns: mismatch\nDone."
        s = agent.analyze(log)
        assert s.error_class == "uvm_error"
        assert s.debug_required is False  # only one UVM_ERROR

    def test_uvm_error_multiple_needs_debug(self, agent: LogAnalyzerService) -> None:
        log = "UVM_ERROR @ 10ns: first\nUVM_ERROR @ 20ns: second"
        s = agent.analyze(log)
        assert s.error_class == "uvm_error"
        assert s.debug_required is True

    def test_scoreboard_mismatch(self, agent: LogAnalyzerService) -> None:
        log = "INFO: transaction sent\nScoreboard mismatch: got 0xAA expected 0xBB"
        s = agent.analyze(log)
        assert s.error_class == "scoreboard_mismatch"

    def test_cocotb_assertion_error(self, agent: LogAnalyzerService) -> None:
        log = "Traceback (most recent call last):\n  File 'test.py'\nAssertionError: expected True"
        s = agent.analyze(log)
        assert s.error_class == "cocotb_error"

    def test_sim_timeout(self, agent: LogAnalyzerService) -> None:
        log = "Running test...\nSimTimeoutError: test exceeded 10000ns"
        s = agent.analyze(log)
        assert s.error_class == "timeout"

    def test_unknown_no_pattern_match(self, agent: LogAnalyzerService) -> None:
        log = "Everything looks fine.\nSimulation complete."
        s = agent.analyze(log)
        assert s.error_class == "unknown"
        assert s.debug_required is True

    def test_message_trimmed_to_120_chars(self, agent: LogAnalyzerService) -> None:
        long_line = "UVM_FATAL @ 1ns: " + "x" * 200
        s = agent.analyze(long_line)
        assert len(s.message) <= 120

    def test_context_window_included(self, agent: LogAnalyzerService) -> None:
        log = "line before\nUVM_FATAL @ 10ns: bad\nline after\nmore"
        s = agent.analyze(log)
...
class TestOutputFormat:
    def test_to_str_contains_sections(self, agent: LogAnalyzerService) -> None:
    ...
    def test_compile_error_recommends_code_generator(self, agent: LogAnalyzerService) -> None:
        s = agent.analyze("*E,NOIPRT: port not found")
        assert "Code Generator" in s.next_step

    def test_debug_required_yes_in_output(self, agent: LogAnalyzerService) -> None:
...
class TestFileInput:
    def test_reads_file_if_path_exists(self, agent: LogAnalyzerService, tmp_path: Path) -> None:
        log_file = tmp_path / "sim.log"
        log_file.write_text("UVM_FATAL @ 5ns: crash\n")
        s = agent.analyze(str(log_file))
        assert s.error_class == "uvm_fatal"

    def test_treats_nonexistent_path_as_content(self, agent: LogAnalyzerService) -> None:
...
class TestAsyncRun:
    @pytest.mark.asyncio
    async def test_run_returns_string(self, agent: LogAnalyzerService) -> None:
