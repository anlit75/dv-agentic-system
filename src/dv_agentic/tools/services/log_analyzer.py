...
"""Simulation log analysis service.
...
analysis) — this service never speculates on root cause.
"""
...
class FailureSummary:
    """Structured result produced by :class:`LogAnalyzerService`.
...
class LogAnalyzerService:
    ...
    Args:
        wiki_config: Optional wiki configuration for ingesting patterns.
    """

    def __init__(self, wiki_config: WikiConfig | None = None) -> None:
    ...
    def _get_lines(task_input: str) -> Iterator[str]:
        ...
        if p.exists() and p.is_file():
            logger.info("LogAnalyzerService reading log from %s", p)
    ...
    async def _ingest_to_wiki(self, summary: FailureSummary) -> None:
        ...
        try:
            ...
            logger.debug("LogAnalyzerService: wiki ingest failed (non-fatal)", exc_info=True)
