...
class VerilatorAdapter(SimulatorTool):
    ...
    def __init__(
        ...
        self.runner = get_runner("verilator")

    def compile(self, file_list: list[str], top: str) -> CompileResult:
        """Compile source files using Verilator via cocotb runner."""
        self.hdl_toplevel = top
        try:
            # Verilator often needs extra arguments like --trace or --cc
            # cocotb runner handles most of these by default.
            self.runner.build(
                verilog_sources=file_list,
                hdl_toplevel=top,
                always=True,
            )
            return CompileResult(status="pass", output="Verilator build successful.")
        except Exception as e:
            logger.exception("Verilator build failed")
    ...
    def run(self, test: str, seed: int, debug: bool) -> SimResult:
        ...
        try:
            ...
            logger.exception("Verilator simulation failed for test '%s'", test)
            return SimResult(
                status="fail",
                job_id=f"{test}_{seed}",
                log_path=log_path,
